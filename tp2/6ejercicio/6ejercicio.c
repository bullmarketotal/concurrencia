...
sem_t puente,mutex, limite_puente; 
...
void* cruzar_auto(void* arg, int direccion_auto) 
{
    ...
    if (direccion_auto == 0)
     {
        printf("Auto %d del norte esta cruzando el puente.\n", id);
    } else {
        printf("Auto %d del sur esta cruzando el puente.\n", id);
    }
    ...
    sem_post(&mutex);
    sem_post(&limite_puente);  // Libera un espacio en el puente para otro auto

    if (direccion_auto == 0) {
        printf("Auto %d del norte cruzo el puente.\n", id);
    } else {
